...
class Pipeline:
    ...
    def answer_single_question(self, question: str, kind: str = "string", display_func=None):
        """
        ...
        t0 = time.time()
        out("[计时] 开始初始化 QuestionsProcessor ...")
        processor = QuestionsProcessor(
            vector_db_dir=self.paths.vector_db_dir,
            documents_dir=self.paths.documents_dir,
        ...
        t1 = time.time()
        out(f"[计时] QuestionsProcessor 初始化耗时: {t1-t0:.2f} 秒")
        out("[计时] 开始调用 process_single_question ...")
        answer = processor.process_single_question(question, kind=kind, display_func=display_func)
        t2 = time.time()
        out(f"[计时] process_single_question 推理耗时: {t2-t1:.2f} 秒")
        out(f"[计时] answer_single_question 总耗时: {t2-t0:.2f} 秒")
        return answer
