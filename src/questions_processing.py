...
class QuestionsProcessor:
    ...
    def get_answer_for_company(self, company_name: str, question: str, schema: str, display_func=None) -> dict:
        # 针对单个公司，检索上下文并调用LLM生成答案
        out = display_func if display_func is not None else print
        t0 = time.time()
        ...
        t1 = time.time()
        out(f"[计时] [get_answer_for_company] 检索器初始化耗时: {t1-t0:.2f} 秒")
        if self.full_context:
            ...
            t3 = time.time()
            out(f"[计时] [get_answer_for_company] 检索耗时: {t3-t2:.2f} 秒")
        if not retrieval_results:
            raise ValueError("No relevant context found")
        t4 = time.time()
        rag_context = self._format_retrieval_results(retrieval_results)
        t5 = time.time()
        out(f"[计时] [get_answer_for_company] 构建rag_context耗时: {t5-t4:.2f} 秒")
        ...
        t6 = time.time()
        out(f"[计时] [get_answer_for_company] LLM调用耗时: {t6-t5:.2f} 秒")
        ...
        out(f"[计时] [get_answer_for_company] 总耗时: {t6-t0:.2f} 秒")
        return answer_dict
    ...
    def process_comparative_question(self, question: str, companies: List[str], schema: str, display_func=None) -> dict:
        """
        处理多公司比较类问题：
        1. 先将比较问题重写为单公司问题
        2. 并行处理每个公司
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_company):
                try:
                    ...
                    out(f"Error processing company {company}: {str(e)}")
                    raise
    ...
    def process_single_question(self, question: str, kind: str = "string", display_func=None):
        """
        单条问题推理，返回结构化答案。
        kind: 支持 'string'、'number'、'boolean'、'names' 等
        display_func: 可选，用于输出的函数（如 st.write），不传则用 print
        """
        out = display_func if display_func is not None else print
        t0 = time.time()
        out("[计时] [单问] 开始公司名抽取 ...")
        ...
        if self.new_challenge_pipeline:
            ...
            extracted_companies = re.findall(r'"([^"]*)"', question)
        t1 = time.time()
        out(f"[计时] [单问] 公司名抽取耗时: {t1-t0:.2f} 秒")
        if len(extracted_companies) == 0:
            raise ValueError("No company name found in the question.")
        if len(extracted_companies) == 1:
            company_name = extracted_companies[0]
            out("[计时] [单问] 开始检索与LLM推理 ...")
            t2 = time.time()
            answer_dict = self.get_answer_for_company(company_name=company_name, question=question, schema=kind, display_func=display_func)
            t3 = time.time()
            out(f"[计时] [单问] 检索+LLM推理耗时: {t3-t2:.2f} 秒")
            out(f"[计时] [单问] 总耗时: {t3-t0:.2f} 秒")
            return answer_dict
        else:
            out("[计时] [单问] 开始多公司比较 ...")
            t2 = time.time()
            answer_dict = self.process_comparative_question(question, extracted_companies, kind, display_func=display_func)
            t3 = time.time()
            out(f"[计时] [单问] 多公司比较耗时: {t3-t2:.2f} 秒")
            out(f"[计时] [单问] 总耗时: {t3-t0:.2f} 秒")
            return answer_dict
